   ...
   private List hasInterceptorList;
	...
	private Interceptor getNextInterceptor() {
		if (hasInterceptorList.size() < interceptorList.size()) {
			Interceptor interceptor = (Interceptor) interceptorList.get(hasInterceptorList.size());
		...
		if (!hasExecuted()) {
			...
			if (BeanHelper.hasMethodIgnoreCase(getAction().getClass(), this
				...
				if (method.getParameterTypes().length == 0) {
					executed = true;
					return BeanHelper.invoke(method, getAction(), null);
				} else {
					/**
					 * 尝试按照类型组织参数
					 */
...
/*   public Object invoke(){
     ...
     if (!hasExecuted()) {
       LOG.debug("invocation[DefaultJavaInvocation]:invoke");
       if (BeanHelper.hasMethodIgnoreCase(getAction().getClass(), 
         getMethodName())) {
         Method method = BeanHelper.getMethodIgnoreCase(getAction().getClass(), getMethodName());
         if (method.getParameterTypes().length == 0) {
           this.executed = true;
           return BeanHelper.invoke(method, getAction(), null);
         }
       ...
       LOG.warn("cann't find method[" + getMethodName() + "] in action[" + getClass().getName() + "]");
       return "success";
     }
     return this.result;
   }*/
